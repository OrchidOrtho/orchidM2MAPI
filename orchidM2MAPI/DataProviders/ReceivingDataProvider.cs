...
namespace orchidM2MAPI.DataProviders
{
    public class ReceivingDataProvider : IReceivingDataProvider
    {
        ...
        //    {
        //        string sQuery = "SELECT freceiver AS ReceivingNo, fvendno AS VendorNo, identity_column AS ReceivingId, fpono AS PONumber, fdaterecv AS DateReceived, fcstatus AS Status, ftype AS Type FROM dbo.rcmast WHERE  (dbo.rcmast.freceiver = @receivingNo); SELECT identity_column AS ReceivingItemId, ftype AS ReceivingItemType, fitemno AS ReceivingItemLineNo, fpartno AS PartNo, fpartrev AS PartRev, fac AS Facility, fqtyrecv AS TotalQuantityReceived, flocation AS InventoryLocation, fmeasure AS QuantityUnitofMeasure, fcomments AS Comments, fpoitemno AS POItemNo, finspect AS InspectFlag FROM  dbo.rcitem WHERE  (dbo.rcitem.freceiver = @receivingNo)";
        //        Receiving returnVal = new Receiving();

        //        using (var multi = conn.QueryMultiple(sQuery, new { receivingNo = ReceivingNo }))
        //        {
        //            var receiving = multi.Read<Receiving>().First();
        //            var receivingItems = multi.Read<ReceivingItem>().ToList();

        //            returnVal = receiving;
        //            returnVal.ReceivingItems = receivingItems.ToList();
        //        }
