            ...
            try
            {
                ...
                switch (location)
                {
                    ...
                    case "072":
                        ...
                        sQuery = "SELECT        TOP (100) PERCENT 0 AS JobId, dbo.job.job + '-' + CAST(dbo.job.suffix AS NVARCHAR(5)) AS JobNo, dbo.job.item AS PartNo, dbo.item.revision AS PartRev, '' AS JobType, dbo.job.ord_num AS SalesOrderNo,                          RTRIM(LTRIM(dbo.job.cust_num)) AS CustomerNo, dbo.job.stat AS JobStatus, dbo.job.job_date AS JobDueDate, dbo.job.qty_released AS JobQuantity, 0 AS JobLotId, dbo.jobtran.lot AS LotNo, dbo.job.qty_released AS LotQuantity,                          'EA' AS LotQtyUnitOfMeasure FROM            dbo.item INNER JOIN                          dbo.jobroute INNER JOIN                          dbo.job ON dbo.jobroute.job = dbo.job.job AND dbo.jobroute.suffix = dbo.job.suffix ON dbo.item.item = dbo.job.item LEFT OUTER JOIN                          dbo.wc ON dbo.jobroute.wc = dbo.wc.wc LEFT OUTER JOIN                          dbo.reason RIGHT OUTER JOIN                          dbo.jobtran ON dbo.reason.reason_code = dbo.jobtran.reason_code ON dbo.jobroute.job = dbo.jobtran.job AND dbo.jobroute.suffix = dbo.jobtran.suffix AND dbo.jobroute.oper_num = dbo.jobtran.oper_num WHERE        (dbo.job.job = @jobNo) AND (dbo.job.suffix = @suffixNo)";
                        break;
                    case "032":
                        newJobNo = JobNo;
