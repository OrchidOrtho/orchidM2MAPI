    ...
    public class ShippingInfoDataProvider : IShippingInfoDataProvider
    {
        ...
        public async Task<ShippingAllInfo> GetShippingInfo(string location, string shipperNo)
        {
            using (IDbConnection conn = Connection(location))
            {
                ...
                {
                    case "070":
                        sQuery = "SELECT dbo.shmast.fshipno, dbo.shmast.fshipdate, dbo.shitem.fpartno, dbo.shitem.frev, dbo.shitem.fshipqty, dbo.shitem.fmeasure, dbo.shlotc.fclot, dbo.shlotc.fnlotqty, LEFT(dbo.shitem.fsokey, 6) AS SaleOrderNumber, dbo.somast.fcustpono, dbo.inmastx.fdescript, dbo.soitem.fcustpart, dbo.SOITEM_EXT.CPOLINE AS CustomerPOLineNumber, dbo.shmast.fcnumber AS CustomerNumber, dbo.soitem.fenumber AS SalesOrderLineNumber FROM dbo.shmast INNER JOIN dbo.shitem ON dbo.shmast.fshipno = dbo.shitem.fshipno INNER JOIN dbo.somast ON LEFT(dbo.shitem.fsokey, 6) = dbo.somast.fsono INNER JOIN dbo.shlotc ON dbo.shitem.fshipno = dbo.shlotc.fcshipno AND dbo.shitem.fitemno = dbo.shlotc.fcitemno INNER JOIN dbo.inmastx ON dbo.shitem.fpartno = dbo.inmastx.fpartno AND dbo.shitem.frev = dbo.inmastx.frev INNER JOIN dbo.soitem ON dbo.somast.fsono = dbo.soitem.fsono AND dbo.shitem.fpartno = dbo.soitem.fpartno AND dbo.shitem.frev = dbo.soitem.fpartrev LEFT OUTER JOIN dbo.SOITEM_EXT ON dbo.soitem.identity_column = dbo.SOITEM_EXT.FKey_ID WHERE (dbo.shmast.fshipno = @shipperNo) ORDER BY dbo.shlotc.fclot";
                        break;
                    default:
                        sQuery = "SELECT dbo.shmast.fshipno, dbo.shmast.fshipdate, dbo.shitem.fpartno, dbo.shitem.frev, dbo.shitem.fshipqty, dbo.shitem.fmeasure, dbo.shlotc.fclot, dbo.shlotc.fnlotqty, LEFT(dbo.shitem.fsokey, 6) AS SaleOrderNumber, dbo.somast.fcustpono, dbo.inmast.fdescript, dbo.soitem.fcustpart, dbo.SOITEM_EXT.CPOLINE AS CustomerPOLineNumber, dbo.shmast.fcnumber AS CustomerNumber, dbo.soitem.fenumber AS SalesOrderLineNumber, j.fjobno, j.LASRETCH FROM dbo.shmast INNER JOIN dbo.shitem ON dbo.shmast.fshipno = dbo.shitem.fshipno INNER JOIN dbo.somast ON LEFT(dbo.shitem.fsokey, 6) = dbo.somast.fsono INNER JOIN dbo.shlotc ON dbo.shitem.fshipno = dbo.shlotc.fcshipno AND dbo.shitem.fitemno = dbo.shlotc.fcitemno INNER JOIN dbo.inmast ON dbo.shitem.fpartno = dbo.inmast.fpartno AND dbo.shitem.frev = dbo.inmast.frev INNER JOIN dbo.soitem ON dbo.somast.fsono = dbo.soitem.fsono AND dbo.shitem.fpartno = dbo.soitem.fpartno AND dbo.shitem.frev = dbo.soitem.fpartrev LEFT OUTER JOIN dbo.SOITEM_EXT ON dbo.soitem.identity_column = dbo.SOITEM_EXT.FKey_ID LEFT OUTER JOIN (SELECT dbo.jomast.fjobno, dbo.JOMAST_EXT.LASRETCH, dbo.qalotc.fclot FROM dbo.JOMAST_EXT INNER JOIN dbo.jomast ON dbo.JOMAST_EXT.FKey_ID = dbo.jomast.identity_column INNER JOIN dbo.qalotc ON dbo.jomast.fjobno = dbo.qalotc.fcdoc WHERE (dbo.qalotc.fctype = 'J')) AS j ON dbo.shlotc.fclot = j.fclot WHERE (dbo.shmast.fshipno = @shipperNo) ORDER BY dbo.shlotc.fclot";
                        break;
                }
